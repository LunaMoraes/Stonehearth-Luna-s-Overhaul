...
App.StonehearthTownView = App.View.extend({
   ...
   init: function() {
      ...
      // Call our custom handler to get elapsed days
      radiant.call('luna_overhaul:get_elapsed_days')
         .done(function (response) {
            if (self.isDestroyed || self.isDestroying) {
               return;
            }
            self.set('elapsed_days', response.elapsed_days);
         });
   ...
   _townDataChanged: function(data) {
      ...
      console.log(data);
      // Set elapsed_days if available
      if (data.elapsed_days !== undefined) {
         self.set('elapsed_days', data.elapsed_days);
      }
   },
